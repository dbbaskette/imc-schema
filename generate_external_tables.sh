...
# Generate vehicle_telemetry_data.sql by substituting placeholders in the template
echo "-- Auto-generated by generate_external_tables.sh from vehicle_telemetry_data.sql" > "$SCRIPT_DIR/vehicle_telemetry_data_generated.sql"
sed -e "s|__TELEMETRY_PATH__|${TELEMETRY_PATH_ONLY}|g" \
    -e "s|__PXF_PROFILE__|${PXF_PROFILE:-hdfs:parquet}|g" \
    -e "s|__PXF_SERVER__|${PXF_SERVER_NAME:-default}|g" \
    "$SCRIPT_DIR/vehicle_telemetry_data.sql" >> "$SCRIPT_DIR/vehicle_telemetry_data_generated.sql"
...
CREATE EXTERNAL TABLE crash_reports_data (
  ...
  vin VARCHAR(17),
  crash_timestamp TIMESTAMP,       -- our parquet has accident_timestamp; name mismatch is fine if column order matches
  crash_type VARCHAR(50),          -- our parquet has accident_type
...
-- Auto-generated by generate_external_tables.sh
\set ON_ERROR_STOP on
\echo '--- Dropping foreign table vehicle_telemetry_data_v2 (if exists) ---'
DROP FOREIGN TABLE IF EXISTS vehicle_telemetry_data_v2 CASCADE;
...
echo "  - vehicle_telemetry_data_v2_generated.sql"